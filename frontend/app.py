...
def recurso_bloqueado(planos):
    r = requests.get(f"{API}/me", headers=get_headers())

    if r.status_code != 200:
        st.error("Erro ao validar plano.")
        st.stop()

    plano = r.json().get("plan", "free")

    if plano not in planos:
        st.warning("🔒 Esta funcionalidade não está disponível no seu plano.")
...
with tab1:
    st.header("🗺️ Talhões")
    st.success("Disponível para todos os planos.")

with tab2:
    recurso_bloqueado(["pro", "enterprise"])
    st.header("🔵 Buffer")
...
with tab4:
    recurso_bloqueado(["enterprise"])
    st.header("🎯 Taxa Variável")

with tab5:
    recurso_bloqueado(["pro", "enterprise"])
    st.header("🌾 Projeção de Linha")
